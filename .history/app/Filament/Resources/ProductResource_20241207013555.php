            ...
            ->columns([
                ...
                TextColumn::make('stock')
                    ->state(function (Product $record): string {
                        $orderitems = OrderItem::query()->leftJoin('orders', 'order_items.id', '=', 'orders.id')->leftJoin('porders', 'order_items.id', '=', 'porders.id')->get()->where('order.status', '!=', 'canceled')->where('porder.status', '!=', 'canceled');
                        $beli = $orderitems->where('product_id', $record->id)->sum('p_quantity');
                ...
                TextColumn::make('alert')
                    ->state(function (Product $record): string {
                        $beli = OrderItem::where('product_id', $record->id)->where('mutation_type', 'Purchase')->sum('p_quantity');
                        $jual = OrderItem::where('product_id', $record->id)->where('mutation_type', 'Sales')->sum('quantity');
                        $hasil = $beli - $jual - $record->low_alert;
                        if ($hasil <= 0) {
                            return "LOW";
                        } else {
                            return "aman";
