            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('order.total_cashback')
                    ...
                    ->state(function (Payment $record): int {
                        ...
                        $orderDate = Order::where('id', $record->order_id)->value('paid_at');
                        $total_cashback = Order::where('id', $record->order_id)->value('total_cashback');
                        if ($payment_method === 'cash' && $date_payment == $orderDate) {
                            $kembali = $total_cashback;
                        } else {
                            $kembali = 0;
                        }
                        return $kembali;
                    ...
                    ->summarize(Sum::make()
                        ->using(fn(Builder $query): string => $query->min('order.total_cashback'))
                        ->numeric(locale: 'nl')->prefix('Rp ')->label('Total')),
                ...
                Tables\Columns\TextColumn::make('porder.total_cashback')
                    ...
                    ->state(function (Payment $record): int {
                        $date_payment = $record->date_payment;
                        $payment_method = $record->payment_method;
                        $orderDate = Porder::where('id', $record->porder_id)->value('paid_at');
                        $total_cashback = Porder::where('id', $record->porder_id)->value('total_cashback');
                        if ($payment_method === 'cash' && $date_payment == $orderDate) {
                            $kembali = $total_cashback;
                        } else {
                            $kembali = 0;
