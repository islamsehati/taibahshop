...
class PorderResource extends Resource
{
                        ...
                        Repeater::make('items')
                            ...
                            ->schema([
                                ...
                                Select::make('product_id')
                                    ...
                                    ->getOptionLabelFromRecordUsing(fn(Model $record) => "{$record->name} {$record->variant} Rp" . Number::format($record->cogs, locale: 'de'))
                    ...
                    Section::make('Payments')->schema([
                        ...
                        Group::make()->schema([
                            ...
                            Placeholder::make('total_cashback_placeholder')
                                ...
                                ->content(function (Get $get, Set $set, Model $record) {
                                    ...
                                    foreach ($repeaters as $key => $repeater) {
                                        $subtotal_payment += $get("payments.{$key}.nominal_mins");
                                    }
                                    ...
                                    if ($total_cashback >= 0) {
                                        $set('is_paid', true);
                                        $set('paid_at', Payment::where('order_id', $record->id)->orderBy('date_payment', 'desc')->latest()->get()->value('date_payment'));
                                    } else {
                                        $set('is_paid', false);
                                        $set('paid_at', null);
                                    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->recordUrl(fn(Model $record): string => PorderResource::getUrl('edit', ['record' => $record]))
