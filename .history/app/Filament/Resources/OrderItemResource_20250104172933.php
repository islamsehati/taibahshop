    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->query(function (OrderItem $query) {
                if (Auth::user()->id != 0) {
                    return $query->where('branch_id', Auth::user()->branch_id)->where('order.status', '!=', 'canceled')->where('porder.status', '!=', 'canceled');
                } else {
                    return $query->where('order.status', '!=', 'canceled')->where('porder.status', '!=', 'canceled');
                    // return $query
