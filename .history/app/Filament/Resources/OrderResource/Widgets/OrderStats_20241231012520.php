...
class OrderStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $today = Carbon::today()->sub('3 day')->format('Y-m-d');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', 'like', "$today")->sum('grand_total');
