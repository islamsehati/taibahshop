...
class OrderStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today%")->sum('grand_total');
        $paid_chart2 = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today2%")->sum('grand_total');
        $paid_chart3 = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today3%")->sum('grand_total');
        $paid_chart4 = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today4%")->sum('grand_total');
        $paid_chart5 = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today5%")->sum('grand_total');
        $paid_chart6 = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today6%")->sum('grand_total');
        $paid_chart7 = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('paid_at', 'like', "%$today7%")->sum('grand_total');

        $incOrDecPaid = ($paid_chart4 >= $paid_chart5) ? 'increase' : 'decrease';
        $incOrDecPaidColor = ($paid_chart4 >= $paid_chart5) ? 'info' : 'danger';
        $incOrDecPaidIcon = ($paid_chart4 >= $paid_chart5) ? 'heroicon-m-arrow-trending-up' : 'heroicon-m-arrow-trending-down';
        $incOrDecPaidPersen = Number::percentage((($paid_chart5 - $paid_chart4) / $paid_chart5) * 100, maxPrecision: 2);
