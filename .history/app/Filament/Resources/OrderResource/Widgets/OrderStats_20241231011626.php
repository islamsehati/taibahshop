...
class OrderStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', 'like', Carbon::today()->format('Y-m-d'))->sum('grand_total');
