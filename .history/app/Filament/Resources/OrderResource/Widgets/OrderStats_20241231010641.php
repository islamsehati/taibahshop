...
class OrderStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today())->sum('grand_total');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today()->sub('1 day'))->sum('grand_total');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today()->sub('2 day'))->sum('grand_total');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today()->sub('3 day'))->sum('grand_total');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today()->sub('4 day'))->sum('grand_total');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today()->sub('5 day'))->sum('grand_total');
        $paid_chart = Order::query()->where('branch_id', Auth::user()->branch_id)->where('is_paid', 1)->where('status', '!=', 'canceled')->where('updated_at', '>=', Carbon::today()->sub('6 day'))->sum('grand_total');
