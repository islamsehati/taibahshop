...
class MyOrdersPage extends Component
{
    ...
    public function render()
    {
        ...
        if ($isadmin == 1) {
            $my_orders = Order::where('branch_id', auth()->user()->branch_id)->orderBy('date_order', 'desc')->paginate(500);
        }
        if ($isadmin == 0) {
        ...
        return view('livewire.my-orders-page', [
            'payments' => $payments,
            'orders' => $my_orders->where('is_paid', true),
            'ordersup' => $my_orders->where('is_paid', false),
