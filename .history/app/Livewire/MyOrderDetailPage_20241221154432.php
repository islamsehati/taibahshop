...
class MyOrderDetailPage extends Component
{
    ...
    public function addPayment($id)
    {
        ...
        if ($this->payment_method == null || $this->input_payment == null) {
            $payment_method = 'cash';
            $input_payment = 0;
        } else {
        ...
        $payment->nominal_plus = Str::replace('.', '', $input_payment);
        ...
        $input_payment =  Str::replace('.', '', $input_payment);
    ...
    public function editPayment($idpay)
    {
        ...
        if ($this->payment_method_edit == null || $this->input_payment_edit == null) {
        ...
        $updatepayment = [
            ...
            'nominal_plus' => Str::replace('.', '', $input_payment_edit),
            'updated_at' => date('Y-m-d H:i:s'),
        ...
        $grand_total = $data->value('grand_total');
        $total_payment = $datapayment = Payment::where('order_id', $orderID)->sum('nominal_plus');
        if ($grand_total - $total_payment <= 0) {
            $paidunpaid = 1;
    ...
    {
        $order_items = OrderItem::with('product')->where('order_id', 47)->get();
        $paymentslast = Payment::where('order_id', 47)->latest()->first();
        $address = Address::where('order_id', 47)->first();
        $address_firstname = Address::where('order_id', 47)->value('first_name');
        $address_lastname = Address::where('order_id', 47)->value('last_name');
        $order = Order::where('id', 47)->get()->first();
        $user = User::all();
        $states = Province::all();
        $cities = City::all();
