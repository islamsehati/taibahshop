...
class MyOrdersPage extends Component
{
    ...
    public function render()
    {
        ...
        if ($isadmin == 1) {
            $my_orders = Order::orderBy('is_paid')->orderBy('date_order', 'desc')->paginate(50);
        ...
        $my_orders_sum_cashback = Order::where('paid_at', 'like', "%$today%")->where('is_paid', 1)->where('total_cashback', '>', 'cash')->sum(value('total_cashback'));

        $payments = Payment::where('created_by', auth()->user()->branch_id)->where('order_id', '!=', null)
